...
public class RunwayLoad implements LoadService{
    ...
    private static String PATH = "classpath:data/runways.csv";
    ...
    public void recordDatas() throws IOException {
        String runwayFile = new String(Files.readAllBytes(getFile(PATH).toPath()));
        ...
        Arrays.stream(runwayLine).forEach(aLine -> {
            ...
            if (!aLine.startsWith("\"id\"")) {
                try {
//                    if (runwayInfo.get(0).contains("249329"))//301544
//                        logger.error("DUR");
//                    logger.error("*** : " + runwayInfo.get(0));
                    Runway newRunway = Runway.builder()
                            .id(Integer.parseInt(runwayInfo.get(0)))
                            .airport_ref( Integer.parseInt(runwayInfo.get(1)) )
                            .airport_ident(!runwayInfo.get(2).equals("") ? runwayInfo.get(2) : "")
